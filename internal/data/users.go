...
// password struct represents passwords.
// Note that plaintext is a pointer to a string, so we can differentiate
// between a password not being present and a password being empty ("").
...
// both the hash and plaintext versions in the password struct.
...
// ValidateEmail runs validation checks against the provided email.
func ValidateEmail(v *validator.Validator, email string) {
	v.Check(email != "", "email", "must be provided")
	v.Check(validator.Matches(email, validator.EmailRX), "email", "must be a valid email address")
}

// ValidatePasswordPlaintext runs validation checks against the provided password.
...
// ValidateUser runs validation checks against the User struct.
...
// GetForToken retrieves the user that the provided token belongs to.
